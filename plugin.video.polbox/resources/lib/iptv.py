...
COOKIEJAR = None
COOKIEFILE = dirname(realpath(__file__)) + sep + 'cookie.%s.txt' % IPTV_DOMAIN
LASTLISTFILE = dirname(realpath(__file__)) + sep + 'last.%s.json' % IPTV_DOMAIN

try:											# Let's see if cookielib is available
	import cookielib
except ImportError:
	xbmc.log('[PolBoxTV] cookielib is not available..')
	pass
else:
...
try:
	...
	xbmc.log('[PolBoxTV] module json is not available. using demjson')
	import demjson
...
class polbox:
	...
	def __init__(self, login, password, addonid = '', SID = None, SID_NAME = None):
		...
		if os.path.isfile(COOKIEFILE):
			try:
				...
				xbmc.log('[PolBox.TV] cookie file is broken. Deleting..')
	...
	def _request(self, cmd, params, inauth = None):
		...
		xbmc.log('[PolBox.TV] REQUESTING: %s' % url)
		...
		xbmc.log('[PolBox.TV] Got %s' % rez, level=xbmc.LOGDEBUG)

		try:
			res = JSONDECODE(rez)
		except:
			xbmc.log('[PolBox.TV] Error.. :(')

		#xbmc.log('[PolBox.TV] Got JSON: %s' % res)

		self._errors_check(res)

		if COOKIEJAR != None:
			xbmc.log('[PolBox.TV] Saving cookies: %s' % COOKIEFILE)
			COOKIEJAR.save(COOKIEFILE)

		return res

	def _auth(self, user, password):
		response = self._request('login', 'login=%s&pass=%s&device=apple' % (user, password), 1)
		self.AUTH_OK = False
		...
		if 'sid' in response:
			...
			if COOKIEJAR != None:
				try:
					...
					xbmc.log('[PolBox.TV] pipec kakaja strannaja shnyaga slu4ilas, pacany!', xbmc.LOGWARNING)
	...
	def getLast(self):
		if self.last_list and 'ttl' in self.last_list:
			if int(self.last_list['ttl']) < time():
				xbmc.log('[PolBox.TV] Last list expired')
				self.last_list = None
		if not self.last_list:
			...
			try:
				...
				xbmc.log('[PolBox.TV] Error loading last list %s' % e)
				res = self.getChannelsList()
			else:
				xbmc.log('[PolBox.TV] last list loaded')
			f.close()
		...
		if self.channels_ttl < time():
			...
			for channelGroup in jsonChannels['groups']:
				...
				for channel in channelGroup['channels']:
					...
					if 'icon' in channel:
						icon = 'http://' + IPTV_DOMAIN + '/dune/polbox/images_v3/'+ channel['id'] +'.5.png'
					else:
						icon = ''
			...
			try:
				...
				xbmc.log('[PolBox.TV] Error saving last list %s' % e)
			else:
				xbmc.log('[PolBox.TV] last list stored')
			f.close()
	...
	def getVideoList(self, mode, page, genre, pagesize=15, search={}):
		if pagesize == 'all':
			...
			xbmc.log('[PolBox.TV] pagesize set to %s to reflect "all" param' % pagesize)
		...
		for vod in result['rows']:
			if 'poster' in vod:
				...
				icon = ''
			xbmc.log('[PolBox.TV] VOD item: %s' % vod, level=xbmc.LOGDEBUG)
			res.append({
				'title':		vod['name'],
	...
	def getSettingsList(self):
		...
		xbmc.log('[PolBox.TV] settings: %s' % res)
		return res
